...
{
    public class LobbyTestingPhoton: MonoBehaviour
    {
    ...
    private async Task ConnectToHost(Player loggedInPlayer, Lobby lobby)
    {
        ...
        var transport = NetworkManager.Singleton.gameObject.GetComponentInChildren<PhotonRealtimeTransport>();
        transport.RoomName = joinCode.Value;
    ...
    private async Task CreateLobbyAndHostGame(Player loggedInPlayer)
    {
        ...
        var joinCode = transport.RoomName;
        Debug.Log($"We created lobby with join-code: {transport.RoomName}");
